"""Tests for EncryptedStorage — encryption/decryption of values and DB."""
...
class TestEncryptedStorage:
    """Tests for EncryptedStorage — XOR-based encryption with PBKDF2 key derivation."""

    def test_default_key(self):
        from tantra.core.encryption import EncryptedStorage
        es = EncryptedStorage()
        assert es is not None
        assert len(es._salt) == 16
    ...
    def test_decrypt_wrong_key_fails(self):
        ...
        decrypted = es2.decrypt_value(encrypted)
        assert decrypted != "secret"  # Wrong key produces garbage
