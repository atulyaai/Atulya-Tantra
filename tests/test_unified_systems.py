...
class TestMemoryOrchestrator:
    def test_register_provider(self):
        from memory.orchestrator import MemoryOrchestrator, MemoryProvider, MemoryEntry
...
class TestSessionSearch:
    def test_store_and_search(self):
        from memory.session_search import SessionSearchProvider
...
class TestSubconscious:
    def test_log_decision(self):
        from memory.subconscious import SubconsciousProvider
        from memory.orchestrator import MemoryEntry
        async def run():
            with tempfile.TemporaryDirectory() as tmp:
...
class TestReflection:
    def test_add_reflection(self):
        from memory.reflection import ReflectionProvider
        async def run():
            with tempfile.TemporaryDirectory() as tmp:
...
class TestChannelRegistry:
    def test_register_and_status(self):
        from yantra.saarthi.channels.unified import ChannelRegistry, WebChatChannel
        reg = ChannelRegistry()
...
class TestObsidianExporter:
    def test_export(self):
        from memory.obsidian import ObsidianExporter
        with tempfile.TemporaryDirectory() as tmp:
...
class TestTaskBrain:
    def test_create_and_update(self):
        from yantra.saarthi.task_brain import TaskBrain, TaskType, TaskState
        with tempfile.TemporaryDirectory() as tmp:
            tb = TaskBrain(tmp)
    ...
    def test_stats(self):
        from yantra.saarthi.task_brain import TaskBrain, TaskType
        with tempfile.TemporaryDirectory() as tmp:
            tb = TaskBrain(tmp)
...
class TestCronScheduler:
    def test_add_and_list(self):
        from yantra.saarthi.cron import CronScheduler
        with tempfile.TemporaryDirectory() as tmp:
            c = CronScheduler(tmp)
...
class TestSourceIngestion:
    def test_markdown_vault(self):
        from yantra.saarthi.sources import MarkdownVaultIngestor
        with tempfile.TemporaryDirectory() as tmp:
