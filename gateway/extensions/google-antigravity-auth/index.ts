...
// OAuth application credentials are deployment configuration, never source code.
...
function requireOAuthCredentials(): { clientId: string; clientSecret: string } {
  if (!CLIENT_ID || !CLIENT_SECRET) {
    throw new Error(
      "Google Antigravity OAuth requires GOOGLE_ANTIGRAVITY_CLIENT_ID and GOOGLE_ANTIGRAVITY_CLIENT_SECRET.",
    );
  }
...
function buildAuthUrl(params: { challenge: string; state: string; clientId: string }): string {
...
async function loginAntigravity(params: {
  ...
  const credentials = requireOAuthCredentials();
  const { verifier, challenge } = generatePkce();
  const state = randomBytes(16).toString("hex");
  const authUrl = buildAuthUrl({ challenge, state, clientId: credentials.clientId });
  ...
  const tokens = await exchangeCode({ code, verifier, ...credentials });
