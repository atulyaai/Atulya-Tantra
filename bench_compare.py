...
from tantra.npdna.config import StrandConfig
from tantra.npdna.genome import Genome
...
from tantra.npdna.strand import Strand as NewStrand
...
def main():
    ...
    for B in [1, 2, 4, 8, 16, 32, 64]:
        ...
        # OLD
        for _ in range(WARMUP): old_s(x, weights=weights)
        start = time.perf_counter()
        for _ in range(ITERS): old_s(x, weights=weights)
        t_old = (time.perf_counter() - start) / ITERS

        # NEW
        for _ in range(WARMUP): new_s(x, weights=weights)
        start = time.perf_counter()
        for _ in range(ITERS): new_s(x, weights=weights)
