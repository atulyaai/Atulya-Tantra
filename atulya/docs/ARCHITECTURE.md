...
**Eviction:** Three mechanisms manage storage limits:
- **On-store eviction** (`_evict_least_used`): when `store()` would exceed `max_entries`, the entry with the lowest `_importance_score()` is popped.
- **Bulk prune** (`prune_by_importance`): sorts all entries by importance score descending, keeps only the top `max_entries`.
- **Sleep cycle** (`sleep_cycle`): after merging similar entries, drops lowest-access-count entries if size still exceeds capacity.

Importance score formula: `access_count + len(related)*0.25 + len(topics)*0.1 - age_hours*0.01` — balances recency, access frequency, and relational richness.
...
**Implemented growth hooks:**
- Auto-add new Strands when plateau/overload policy triggers
- Importance-based Cortex pruning

**Longer-term extension:**
- Auto-add new Mesh layers when sustained loss plateaus require depth, not only strand capacity
...
### Voice (Phase 2)
```python
# In tantra/npdna/encoder_audio.py:
...
### Vision (Phase 3)
```python
# In tantra/npdna/encoder_vision.py:
