...
class CronScheduler:
    ...
    def _parse_schedule(self, schedule: str) -> float:
        ...
        try:
            ...
            return max(0.0, self._next_run(schedule) - time.time())

    def _next_run(self, schedule: str) -> float:
        try:
            return time.time() + float(schedule)
        except ValueError:
            if croniter is None:
                raise ValueError("croniter is required for cron expressions")
            return croniter(schedule, time.time()).get_next(float)
