...
class SensorManifest:
    ...
    def __init__(self):
        self.registry = {
            ...
            "WEB": {"state": "BUFFER", "priority_base": 7, "quota": 10, "cycle_count": 0, "starvation": 0}
        }
...
class WebSensor:
    """Listens for commands via the EventBus (FastAPI Bridge)."""
    def __init__(self, manifest):
        self.manifest = manifest
    ...
    def _on_event(self, event_type, payload):
        if event_type == "external_command":
    ...
    def capture(self):
        if self.command_queue.empty():
            return None
        raw_command = self.command_queue.get()
        return self.manifest.normalize("WEB", raw_command, is_interrupt=True)
